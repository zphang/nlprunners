...
class Example(BaseExample):
    ...
    def to_feature_list(self, tokenizer, feat_spec: FeaturizationSpec,
        ...
        if is_training and self.is_impossible:
        ...
        max_tokens_for_doc = feat_spec.max_seq_length - len(query_tokens) - 3
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if not feat_spec.cls_token_at_end:
                ...
                segment_ids.append(feat_spec.cls_token_segment_id)
                p_mask.append(0)
                cls_index = 0
            ...
            for i in range(doc_span.length):
                ...
                segment_ids.append(feat_spec.sequence_b_segment_id)
                p_mask.append(0)
            ...
            while len(input_ids) < feat_spec.max_seq_length:
                input_ids.append(tokenizer.pad_token)
                input_mask.append(0 if mask_padding_with_zero else 1)
                segment_ids.append(feat_spec.pad_token_segment_id)
                p_mask.append(1)

            assert len(input_ids) == feat_spec.max_seq_length
            assert len(input_mask) == feat_spec.max_seq_length
            assert len(segment_ids) == feat_spec.max_seq_length
            ...
            features_list.append(
                DataRow(
                    unique_id=f"{self.qas_id}-{doc_span_index}",
                    qas_id=self.qas_id,
...
class SquadTask(Task):
    ...
    def __init__(self, name, path_dict, squad_version):
    ...
    @classmethod
    def read_squad_examples(cls, path, set_type, squad_version):
