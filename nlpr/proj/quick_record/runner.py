...
class SimpleTaskRunner(BaseRunner):
    ...
    def run_train_step(self, batch, batch_metadata, train_global_state):
        self.model.train()
        ...
        self.log_writer.write_entry("loss_train", {
            ...
            "preds": [int(x) for x in logits.detach().cpu().numpy().argmax(axis=1)],
...
class ModifiedMetaRunner(metarunner.MetaRunner):

    def inject_at_step(self):
        if self.train_global_state.global_step % 100 == 0:
            logits = self.runner.run_test(self.train_cache)
            save_fol = os.path.join(self.output_dir, "train_status")
            os.makedirs(save_fol, exist_ok=True)
            evaluate.write_preds(
                logits=logits,
                output_path=os.path.join(save_fol, f"{self.train_global_state.global_step:010d}.csv"),
            )
